...
int **A;
int **B;
//C is the result of serial Strassen
int **C;
//C_p is the result of parallel Strassen.
int **C_p;
// Reference matrix, call simpleMM to populate.
int **R;


// M_1 - M_7 for strassen intermediate M result
int **M_1;
int **M_2;
int **M_3;
int **M_4;
...
//cal the half size of N.
int sub_size;
//zero: a sub_size * sub_size matrix filled with zero.
int **zero;
...
// matrix add, 
// i_1,j_1 for first matrix index,
// i_2,j_2 for second matrix index
...
int main(int argc, char* argv[]) {
  ...
  timerStart();
  strassenMM(N);
  printf("Serial Strassen MM took %ld ms\n", timerStop());

  timerStart();
  // paralle strassenMM
  for (int i=0; i < 7; i++) {
    ranks[i] = i;
    //Create the thread(s) here, calling paralle_strassenMM.
    //thread 0 -  6calculate M_1 - M_7
    pthread_create(&ids[i], NULL, paralle_strassenMM, &ranks[i]);
  }
  for (int i=0; i < 7; i++) {
    // Join thread0 - thread6  here.
    pthread_join(ids[i], NULL);
  }
  for (int i=7; i < 11; i++) {
    ranks[i] = i;
    //Create the thread(s) here, calling paralle_strassenMM.
    //thread 7 - 10 calculate C_1 - C_4
    pthread_create(&ids[i], NULL, paralle_strassenMM, &ranks[i]);
  }
  for (int i=7; i < 11; i++) {
    //Join thread7 - thread10  here.
    pthread_join(ids[i], NULL);
  }
  printf("Parallel Strassen MM took %ld ms\n", timerStop());

  //judge the result of C and R
  ...
  if (compareMatrix(C, C_p, N) != 0) {
    ...
    printf("Matrix C_p doesn't match Matrix C, if N < 20 they will be printed above.\n");
  }
  // stopping timer
